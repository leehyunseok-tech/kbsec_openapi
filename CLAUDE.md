...
- 프로젝트 루트의 통합 런처 `run-kbsec-openapi.bat`/`.sh`(인자: `telegram`/`terminal`/`web [token]`, 기본 `web`, `help`/`-h`/`--help`로 사용법 안내)는 `manage/run/run-*.*`를 감싸는 래퍼일 뿐이며 **gitignore 대상 — `manage/install/install-project.*` 실행 시 OS에 맞게 생성되는 로컬 산출물**이다. 저장소에 없다고 지우거나 다시 만들어 커밋하지 말 것. 이 둘의 소스는 저장소에 직접 존재하지 않고 `manage/install/install-project.bat`(base64 blob)/`.sh`(heredoc) 안에 있으므로, 사용법 문구를 고칠 때는 로컬 `run-kbsec-openapi.bat`를 먼저 고친 뒤 그 바이트를 재인코딩해 blob에 반영해야 한다(절차는 `install-project.bat`의 5) 섹션 주석 참고).
- 프로젝트 루트에는 파라미터 없이 더블클릭만으로 실행되는 클라이언트별 숏컷도 있다: `웹-실행.bat`/`.sh`, `터미널-실행.bat`/`.sh`, `텔레그램-실행.bat`/`.sh` — 각각 `manage/run/run-{web,terminal,telegram}.*`를 그대로 호출하는 얇은 래퍼이며, 위 통합 런처와 동일하게 **gitignore 대상**이고 소스는 `install-project.bat`(6) 섹션, `-EncodedCommand` 사용 — 아래 인코딩 주의사항 참고)/`.sh`(heredoc 루프) 안에 있다. 종료는 실행된 창의 X 버튼(또는 Ctrl+C)이면 충분하므로 별도 종료 스크립트는 두지 않는다.
- **Windows 배치파일에서 한글 `echo`를 쓸 때는 반드시 `chcp 65001 >nul`을 파일 최상단(`@echo off` 바로 다음)에 넣고, 줄바꿈은 CRLF를 유지해야 한다** — UTF-8(BOM 유무 무관)이어도 LF만 있으면 cmd.exe가 이후 라인 파싱에서 깨진다(직접 재현 확인됨: `chcp 65001` + LF만 조합 시 "not recognized" 오류가 라인을 넘나들며 연쇄적으로 발생, CRLF로 바꾸면 즉시 해결). `manage/run/run-*.bat` 3종이 이 패턴을 따른다. 파일명 자체에 한글이 들어가는 경우(`웹-실행.bat` 등을 `install-project.bat`가 생성하는 경우)는 cmd의 콘솔 코드페이지에 의존하는 일반 커맨드라인 인자 대신 `powershell -EncodedCommand`(UTF-16LE 스크립트를 base64로 인코딩)를 쓴다 — 코드페이지와 무관하게 항상 정확하다.
