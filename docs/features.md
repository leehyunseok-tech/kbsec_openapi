...
`manage/generate/generate_api_client.py`가 생성하는 `src/api/registry.py`(`CODE_TO_MODULE`을
...
| 기능 | 상태 | 담당 파일 | 비고 |
|---|---|---|---|
| API 직접 실행 (`/api {코드}`) | ✅ | `src/commands/api_command.py`, `src/utils/api_spec.py` | INPUT 표에서 설명에 "코드:라벨" 선택지가 있는 필드(필수여부 무관)는 번호로 선택(대화형), 그 외는 타입(길이)만큼 공백으로 자동 채움. **필수여부(Y/N)는 부정확해 무시**(누락 검증도 안 함) |
...
### 2.2 API 전체 자동 실행 커맨드 (74개, 코드/이름 기반 전용 슬래시 명령)

`docs/api/md` 하위 74개 API(국내 47 + 해외 27, OAuth 2개 제외) 전부를 `/api {코드}`의
대화형 번호 선택 없이 "커맨드 하나 = API 하나"로 즉시 실행하는 전용 명령. `/api`와
같은 `api_spec.py` 파싱 결과를 재사용하지만, INPUT 표의 **모든 입력 필드**를 표 순서
그대로 위치 파라미터로 받아(앞에서부터 채우고 나머지는 공백 자동 채움) 즉시 실행한다는
점이 다르다 — KB 명세가 조회 API의 핵심 파라미터(종목코드 등)도 필수여부를 `N`으로
표기하는 경우가 많아 필수/선택을 가리지 않고 전부 받는다(예: `/IVM10050-기업개요
000660`). 그래서 매수/매도처럼 종목코드·수량 같은 값도 실제로 넣어 즉시 실행할 수
있다(반대로 그만큼 주문 계열도 그대로 실행된다는 뜻이니 실거래 계좌에서 각별히 주의).

`src/commands/*_command.py`처럼 `commands` 딕셔너리에 74개를 개별 등록하지 않고,
`_dispatch_direct()`의 "알 수 없는 명령어" 폴백 직전에 동적 판정 1곳만 추가해 3개
클라이언트가 전부 지원한다(`/call`·`/info`·`/list`도 같은 방식으로 `commands` 딕셔너리를
우회하는 기존 전례와 동일한 원칙). `docs/command_guide_for_ai.md`에는 의도적으로 등록하지
않아 AI 자연어 변환이 이 커맨드를 스스로 만들어내지 않는다(역시 `/call`·`/info`·`/list`와
같은 전례).

| 기능 | 상태 | 담당 파일 | 비고 |
|---|---|---|---|
| 전용 커맨드 실행 (`/{코드}-{API명}` 공백→`_`) | ✅ | `src/utils/direct_api_command.py` | 3개 클라이언트의 `_dispatch_direct()`가 공통으로 호출. 이름/토큰 목록은 `docs/개발환경/command_summary.md` 9절 참고 |
| 웹 계층 트리 + 클릭 채움 | ✅ | `src/web/static/js/guide-tree.js`, `src/web/static/index.html` | `index.html` "사용 방법" 패널 하단, `api.html`의 `/api/spec/tree`·`/api/spec/detail` 엔드포인트를 그대로 재사용 — 새 백엔드 엔드포인트 없음 |
...
| `command_guide_for_ai.md` 런타임 참조 문서 | ✅ | `docs/command_guide_for_ai.md` | **명령어 변경 시 반드시 같이 갱신** (CLAUDE.md 필수 규칙) |
