...
def skam2101(
    ...
    *,
    ...
    Args:
        ...
        is_cd: 종목코드 — ex)TSLA
        frgn_ordr_typ_cd: 해외주문유형코드 — 1:시장가, 2:지정가, 3:VWAP시장가, 4:TWAP시장가
        frgn_ordr_q: 해외주문수량
...
def skam2201(
    ...
    *,
    ...
    Args:
        trd_dl_ccd: 매매거래구분코드 — 01-매도, 02-매수
        is_cd: 종목코드
        amt_q_clsf: 금액수량구분 — 0-금액, 1-수량
        tv_s_est_f: 전량매도설정여부 — 0-일부매도, 1-전량매도
        frgn_ordr_typ_cd: 해외주문유형코드 — 2-지정가, E-유사시장가
        crncy_ccd: 통화구분코드 — 0-원화, 1-외화(USD)
        ordr_amt: 주문금액
        dcml_ordr_q_p6: 소수점주문수량P6
        frgn_ordr_prc_p4: 해외주문가격P4
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
...
def ssam1801(
    ...
    *,
    ...
    Args:
        ...
        ordr_q: 주문수량
        ordr_uprc: 주문단가
        ordr_ccd: 주문구분코드 — 00:지정가,03:시장가,12:최유리지정가,13:최우선지정가,M3:중간가
        sor_ordr_ccd: SOR주문구분코드 — K:KRX,N:NXT,S:SOR
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    data_body = {
...
def ssam1805(
    ...
    *,
    ...
    Args:
        mkt_tm_clsf: 시장시간구분 — 1:정규장,2:장개시전시간외종가,3:장종료후시간외종가,4:장종료후시간외단일가
        is_cd: 종목코드
        ordr_q: 주문수량 — *일부정정시 입력
        ordr_uprc: 주문단가
        ordr_ccd: 주문구분코드 — 00:지정가,03:시장가,05:조건부지정가,12:최유리지정가,13:최우선지정가
        crct_clsf: 정정구분 — 1:일부정정,2:전부정정
        orgn_ordr_no: 원주문번호
        sor_ordr_ccd: SOR주문구분코드 — K:KRX,N:NXT,S:SOR
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
...
def ssam5762(
    ...
    *,
    ...
    Args:
        ...
        ordr_q_p6: 주문수량P6
        ordr_amt: 주문금액
        dcml_ordr_std_ccd: 소수점주문기준구분코드 — 1:금액, 2:수량 (전량일때 2번으로)
        tv_s_est_f: 전량매도설정여부 — 0:일부매도, 1:전량매도
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
...
def ssam5763(
    ...
    Args:
        ...
        ordr_q_p6: 주문수량P6
        ordr_amt: 주문금액
        dcml_ordr_std_ccd: 소수점주문기준구분코드 — 1:금액,2:수량
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    data_body = {
...
def ssam5764(
    ...
    *,
    ...
    Args:
        ...
        bnf_is_cd: 수익증권종목코드
        trd_dl_ccd: 매매거래구분코드 — 01:매도, 02:매수
        dmstc_stk_dcml_ordr_sq: 국내주식소수점주문일련번호 — SSQM5765 api 의 acpt_no 값
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    data_body = {
