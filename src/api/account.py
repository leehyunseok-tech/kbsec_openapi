...
def spqm2103(ccls_clsf, ordr_dt="", dl_clsf="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        ccls_clsf: 체결구분 (필수) — 1:전체,2:체결,3:미체결
        ordr_dt: 주문일자 (선택)
        dl_clsf: 거래구분 (선택) — 0.전체 1.일반 2.소수점
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
...
def spqm2204(strt_ordr_dt, end_ordr_dt, ccls_clsf, trd_clsf, stnd_is_cd="", dl_clsf="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        strt_ordr_dt: 시작주문일자 (필수)
        end_ordr_dt: 종료주문일자 (필수)
        ccls_clsf: 체결구분 (필수) — 0: 전체, 1: 체결, 2: 미체결
        trd_clsf: 매매구분 (필수) — 99: 전체, 01: 매도, 02: 매수
        stnd_is_cd: 표준종목코드 (선택)
        dl_clsf: 거래구분 (선택) — 0:전체 ,1:일반거래,2:소수점거래
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["strt_ordr_dt", "end_ordr_dt", "ccls_clsf", "trd_clsf"],
        token=token,
        host_url=host_url,
    )
...
def spqm2205(strt_ordr_dt, end_ordr_dt, trd_clsf="", stnd_is_cd="", krw_unty_mgn_rqst_f="", dl_clsf="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        strt_ordr_dt: 시작주문일자 (필수)
        end_ordr_dt: 종료주문일자 (필수)
        trd_clsf: 매매구분 (선택) — 99:전체, 01:매도, 02:매수
        stnd_is_cd: 표준종목코드 (선택)
        krw_unty_mgn_rqst_f: 원화통합증거금신청여부 (선택) — 0: 외화기준, 1: 원화기준
        dl_clsf: 거래구분 (선택) — 0:전체, 1:일반거래, 2:소수점거래
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["strt_ordr_dt", "end_ordr_dt"],
        token=token,
        host_url=host_url,
    )
...
def spqm2206(ordr_dt, stnd_is_cd="", std_crncy_f="", exch_r_aplc_f="", frgn_stk_mgn_ccd="", dl_clsf="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        ordr_dt: 주문일자 (필수)
        stnd_is_cd: 표준종목코드 (선택)
        std_crncy_f: 기준통화여부 (선택) — 1:외화기준,2:원화기준
        exch_r_aplc_f: 환율적용여부 (선택) — 1:환전시매도환율, 2:매매기준율
        frgn_stk_mgn_ccd: 해외주식증거금구분코드 (선택) — 1:원화, 2:외화
        dl_clsf: 거래구분 (선택) — 0:전체,1:일반거래,2:소수점거래
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
...
def spqm2226(std_crncy_f, exch_r_aplc_f="", fee_clsf="", cn_f="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        std_crncy_f: 기준통화여부 (필수) — 1: 외화기준, 2: 원화기준
        exch_r_aplc_f: 환율적용여부 (선택) — 1:환전시매도환율, 2: 매매기준환율(원화)
        fee_clsf: 수수료구분 (선택) — 0: 포함, 1: 미포함
        cn_f: 연속여부 (선택)
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
...
def srqm3051(rgt_clsf, strt_dt="", is_cd="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    """SRQM3051 계좌권리 발생내역 — 계좌에서 발생한 권리 내역을 조회합니다.배당금 지급, 주식 배당, 무상증자 등 보유 종목에서 발생한 권리 행사 내역을 확인할 수 있습니다.
    
    Args:
        rgt_clsf: 권리구분 (필수) — 0:전체, 1:배당, 2:유상증자/BW권리행사, 3:무상증자, 4:매수청구, 5:감자, 6:액면분할/액면병합, 7:피흡수합병
        strt_dt: 시작일자 (선택)
        is_cd: 종목코드 (선택)
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
...
def ssqm1801(inq_clsf="", is_no="", mkt_tm_ccd="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        inq_clsf: 조회구분 (선택) — 0:현금주식매도,1:현금주식예약,2:현금주식일괄매도,3:ELW+현금,4:장외단주매도,5:ELW 전용,6:현금주식매도+코넥스+ETN,7:코넥스전용,8:ETN 전용,9:ELW+현금+ETN
        is_no: 종목번호 (선택)
        mkt_tm_ccd: 시장시간구분코드 (선택) — 1:정규시장,2:장개시전시간외,3:장종료후시간외,4:시간외단일가
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
...
def ssqm2392(is_no="", ordr_dt_from="", ordr_dt_to="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        is_no: 종목번호 (선택)
        ordr_dt_from: 주문일자FROM (선택)
        ordr_dt_to: 주문일자TO (선택)
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
...
def ssqm2442(inq_strt_dt, inq_end_dt, md_clsf, is_cd="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    """SSQM2442 실현손익현황 — 
    
    Args:
        inq_strt_dt: 조회시작일자 (필수)
        inq_end_dt: 조회종료일자 (필수)
        md_clsf: 매체구분 (필수) — 1:오프라인 2:온라인 3:지점
        is_cd: 종목코드 (선택)
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["inq_strt_dt", "inq_end_dt", "md_clsf"],
        token=token,
        host_url=host_url,
    )
...
def ssqm2932(inq_clsf, excg_mktpr_ccd, *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        inq_clsf: 조회구분 (필수) — 1:계좌별, 2:상품유형별 (자문/일임)
        excg_mktpr_ccd: 거래소시세구분코드 (필수) — A:통합시세,K:KRX시세,N:NXT시세
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    data_body = {
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["inq_clsf", "excg_mktpr_ccd"],
        token=token,
        host_url=host_url,
    )
...
def ssqm5765(trd_clsf="", trd_strt_dt="", trd_end_dt="", is_cd="", nxt_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        trd_clsf: 매매구분 (선택) — 0: 전체, 1:매도, 2:매수
        trd_strt_dt: 매매시작일자 (선택)
        trd_end_dt: 매매종료일자 (선택) — * 당일자 조회시에는 빈값
        is_cd: 종목코드 (선택)
        nxt_key: 다음키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
