...
def spao2104(is_cd, trd_dl_ccd, ordr_typ_cd, ordr_q, frgn_ordr_prc_p4, strt_tm="", end_tm="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        ...
        ordr_typ_cd: 주문유형코드 (필수) — 1:시장가, 2:지정가, 3:VWAP시장가, 4:TWAP시장가
        ordr_q: 주문수량 (필수)
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["is_cd", "trd_dl_ccd", "ordr_typ_cd", "ordr_q", "frgn_ordr_prc_p4"],
        token=token,
        host_url=host_url,
    )
...
def spqo2105(rsrv_dt, stnd_is_cd, trd_clsf, krw_unty_mgn_rqst_f="", cn_key="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        ...
        trd_clsf: 매매구분 (필수) — 1:시장가,3:지정가
        krw_unty_mgn_rqst_f: 원화통합증거금신청여부 (선택) — 0: 외화, 1: 원화
        cn_key: 연속키 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["rsrv_dt", "stnd_is_cd", "trd_clsf"],
        token=token,
        host_url=host_url,
    )
...
def ssam0831(ordr_jb_clsf, is_cd, ordr_uprc, ordr_q, ordr_ccd, strt_dt="", end_dt="", mkt_tm_ccd="", *, extra: dict | None = None, token, host_url) -> dict:
    ...
    Args:
        ordr_jb_clsf: 주문업무구분 (필수) — 1:매도,2:매수
        is_cd: 종목코드 (필수)
        ordr_uprc: 주문단가 (필수)
        ordr_q: 주문수량 (필수)
        ordr_ccd: 주문구분코드 (필수) — 00:지정가,03:시장가,12:최유리지정가,13:최우선지정가
        strt_dt: 시작일자 (선택)
        end_dt: 종료일자 (선택)
        mkt_tm_ccd: 시장시간구분코드 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
        token, host_url: 인증 토큰 및 호스트 URL
    """
    data_body = {
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["ordr_jb_clsf", "is_cd", "ordr_uprc", "ordr_q", "ordr_ccd"],
        token=token,
        host_url=host_url,
    )


def ssqm0831(ordr_dt, trd_clsf, hndl_clsf, tv_rv_ccd, nxt_key="", end_dt="", is_cd="", *, extra: dict | None = None, token, host_url) -> dict:
    """SSQM0831 예약주문처리 조회 — 
    
    Args:
        ordr_dt: 주문일자 (필수)
        trd_clsf: 매매구분 (필수) — 0:전체,1:매도,2:매수
        hndl_clsf: 처리구분 (필수) — 0:전체,S:완료,R:접수,E:거부
        tv_rv_ccd: 전량잔량구분코드 (필수) — 0:전체,N:일반,T:기간
        nxt_key: 다음키 (선택)
        end_dt: 종료일자 (선택)
        is_cd: 종목코드 (선택)
        extra: INPUT 표에 없는 추가 dataBody 필드
    ...
    return call_business_api(
        ...
        data_body=data_body,
        required=["ordr_dt", "trd_clsf", "hndl_clsf", "tv_rv_ccd"],
        token=token,
        host_url=host_url,
    )
